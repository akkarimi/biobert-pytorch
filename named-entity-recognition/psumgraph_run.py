...
class PSumGraph(PreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, edge_index=None, token_type_ids=None, labels=None, return_dict=None):
...
def main():
    ...
    #    cache_dir=model_args.cache_dir,
    #)
    model = PSumGraph(model_args.model_name_or_path, 
            from_tf=bool(".ckpt" in model_args.model_name_or_path), 
            config=config, 
            cache_dir=model_args.cache_dir, 
            num_labels=3)
